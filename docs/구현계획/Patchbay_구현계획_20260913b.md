...
| 문서명 | Patchbay_구현계획 |
| 버전 | 20260913b |
| 이전 버전 | `Patchbay_구현계획_20260913a.md` |
| 기반 기획서 | `Patchbay_기획_20260913a.md` |
| 상태 | **Phase 0~3 구현·검토·머지 완료** (커밋 `bca1f1b`..`cb60795`). Phase 4부터 실행 전 |
| 다음 파일명 | 내용이 바뀌면 `Patchbay_구현계획_20260913c.md`처럼 접미사 증가. 날짜가 바뀌면 `Patchbay_구현계획_YYYYMMDDa.md`로 새로 시작 |

기획서(`Patchbay_기획_20260913a.md`)는 "무엇을 만들지"를 고정한다. 이 문서는 "어떤 순서·태스크·완료 조건으로 만들지"를 고정한다. 기획이 바뀌면 이 문서도 다시 검토해야 한다.

20260913b판 변경 요지: 실제 구현 착수(Phase 0~3, 요청서/검토 구조로 진행) 중 나온 두 가지 실행 시점 차이를 반영했다 — (1) **studio 홈 초기화를 Phase 0에서 Phase 3으로 옮김**(Phase 0은 "스텁 이상의 로직 없음" 원칙을 지키고, `patchbay init`이 어차피 같은 시점에 studio.yaml을 만들기 때문), (2) Phase 3의 `ATLAS.md` `## Skills`/`## Rules`/`## Unlabeled ports` 콘텐츠 매핑을 명세 7.18절에 확정. 코드 자체는 이미 이 방식대로 구현·머지됐다 — 이 문서는 뒤늦게 따라잡는 것.
...
| Q2 studio 홈 git 관리 | Phase 3(`patchbay init`)에서 studio 홈 자체를 별도 git 레포로 init (20260913b판: Phase 0에서 옮김) | 낮음 — 나중에 꺼도 됨 |
...
## 3. Phase 0 — 레포·툴체인 부트스트랩 ✅ 완료 (`bca1f1b`)
...
**(20260913b판) studio 홈 초기화는 Phase 3으로 옮김.** 원래 이 절에 "studio 홈 초기화 스크립트: `~/.patchbay`가 없으면 만들고 그 안에서 `git init`" 항목이 있었으나, 실제 구현 시점에 Phase 0은 "스텁 이상의 로직 없음"(실제 `$HOME`을 건드리는 제품 동작이 없어야 함) 원칙과 충돌한다고 판단해 미뤘다. `patchbay init`(Phase 3)이 어차피 같은 시점에 studio.yaml을 만드므로 자연스러운 위치다. 실제로 `packages/core/src/studio/studioStore.ts`의 `ensureStudioInitialized`가 이 역할을 한다.

---

## 4. Phase 1 — 코어 스키마 (Zod) ✅ 완료 (`cbe47cb`, null byte 수정 `a7e3b5f`)
...
## 5. Phase 2 — 결정적 스캐너 ✅ 완료 (`ae8471a`, gitignore 픽스처 수정 `795f304`)
...
## 6. Phase 3 — CLI 최소 + Atlas/Flow 렌더링 (첫 게이트) ✅ 기계적 완료 (`cb60795`, 제품 게이트는 아직)

**목표**: 여기서 멈추고 며칠 실사용한다. MCP·스킬은 아직 없다.

**작업** (`packages/core/src/studio/`, 20260913b판 신설 — CLI가 아니라 Core에 둠, MCP가 나중에 같은 `studio.yaml`을 읽어야 하므로)
- `studioStore.ts` — `resolveStudioHome`(PATCHBAY_HOME/`~/.patchbay`), `ensureStudioInitialized`(디렉터리 생성 + `git init`, 여기로 옮겨온 studio 홈 초기화), `readStudio`/`writeStudio`/`registerProject`
- `projectFiles.ts` — `<studioHome>/projects/<alias>/{overlay.yaml, inferred.json, ATLAS.md, FLOW.md}` 읽기/쓰기. overlay는 없을 때만 생성(사람 소유), 나머지는 항상 덮어씀(커널 소유)

**작업** (`packages/core/src/render/`, `packages/core/src/clarity/`)
- `atlasRenderer.ts` — Inferred+Overlay → `ATLAS.md` (L2). `## Skills`/`## Rules`/`## Unlabeled ports` 콘텐츠 매핑은 명세 7.18절 참고 (온톨로지가 아직 없는 v1 한정 임시 매핑)
- `flowRenderer.ts` — Inferred+Overlay → `FLOW.md` (Mermaid, 아키텍처 모드만). 노드 id는 명세 7.13절 그대로
- `clarityScore.ts` — 명세 7.10절 공식 그대로, 결과는 [0,100]로 클램프

**작업** (`packages/cli/src/`, `commander` 사용)
- `commands/init.ts` — studio.yaml 생성(존재하는 등록 프로젝트 목록은 보존)
- `commands/projectAdd.ts` — clarity 게이트(exit 0/2/1) + 등록 + scan + overlay/inferred/ATLAS/FLOW 기록, 성공 시 ATLAS.md 절대경로 stdout 출력
- `commands/projectNew.ts` — **계약만**(디렉터리·`--path` 검증 + 빈 프로젝트 등록). 대화형 인터뷰는 Phase 9
- `commands/scan.ts` — 등록된 프로젝트 재스캔, overlay는 안 건드림
- `commands/atlas.ts` — 등록된 프로젝트의 ATLAS.md를 stdout에 출력 (`--level`은 Phase 4)

**완료 조건 실측**: `packages/core` 137 테스트, `packages/cli` 39 테스트(built binary against execa 통합 테스트 포함), 전체 빌드 클린. 모든 테스트가 `PATCHBAY_HOME`을 임시 디렉터리로 강제 — 실제 `~/.patchbay`는 건드리지 않음(검토 시 직접 확인).

**게이트는 아직 열지 않았다.** "며칠 실사용 후 포트폴리오 조망이 유용한가"라는 제품 게이트는 기계적 완료와 별개이고, 이 구현 세션이 판단할 수 있는 것도 아니다 — Phase 4 착수 전에 사용자가 직접 판단해야 한다.
