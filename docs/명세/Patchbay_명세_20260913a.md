...
| 문서명 | Patchbay_명세 |
| 버전 | 20260913a |
| 이전 버전 | `Patchbay_명세_20260912d.md` |
| 기반 문서 | `Patchbay_기획_20260913a.md`, `Patchbay_구현계획_20260913a.md` |
| 용도 | 구현 중 계속 열어두고 "지금 하는 게 맞는 방향인지" 점검하는 문서 |
| 갱신 규칙 | 7절(임시 채택값)이 기획·구현계획과 충돌하면 구현 시작 전까지는 7절이 이긴다. 기획/구현계획 다음 개정에서 흡수할 것 |

20260913a판 변경 요지: 기획서에 신설된 **멀티 LLM 역할 분담(파일 매개형)** 기능을 반영했다 — **Phase 12**로 구현계획에 추가, 원칙 자가점검 15번 신설(자동 트리거/폴링 금지 가드), 7.17절에 채택값(동시 갱신 정책, 무응답 감지 범위, MCP 노출 순서) 정리. 새로운 결정은 이 기능 관련 항목뿐이다.
...
- [ ] `FLOW.md`의 Mermaid 노드 id는 **`n` + PortId SHA-1 해시(hex) 앞 12자** — 6.13절과 정확히 동일한 규칙. 언더스코어 없음. 라벨만 경로
- [ ] `project add --path` 비대화형: 성공 시 exit 0, triage 권고 시 exit 2, 프롬프트 없음 (6.6절)
- [ ] `project new`는 `--path` 없으면 exit 1
- [ ] `project add`/`scan` 완료 시 `ATLAS.md`의 **절대경로**를 stdout에 출력 (6.15절, Atlas 가시성 완화)
...
- 어떤 순서로 만들지 → `Patchbay_구현계획_20260913a.md` (7절과 충돌하면 이 문서 7절)
- 지금 맞게 가고 있는지 → 이 문서
