...
| 문서명 | Patchbay_명세 |
| 버전 | 20260912d |
| 이전 버전 | `Patchbay_명세_20260912c.md` |
| 기반 문서 | `Patchbay_기획_20260912c.md`, `Patchbay_구현계획_20260912b.md` |
| 용도 | 구현 중 계속 열어두고 "지금 하는 게 맞는 방향인지" 점검하는 문서 |
| 갱신 규칙 | 7절(임시 채택값)이 기획·구현계획과 충돌하면 구현 시작 전까지는 7절이 이긴다. 기획/구현계획 다음 개정에서 흡수할 것 |

d판 변경 요지: 휴대폰에서 텔레그램으로 LLM에 원격 명령을 보낼 수 있는 기능을 **새 상시 호스트**로 추가했다 — `apps/telegram-bot`, v1은 **읽기 전용**(기존 MCP read 5종만 노출), 화이트리스트 단일 chat id, 수동 기동(자동 기동 없음). 구현계획에는 **Phase 11**로 반영했다(`Patchbay_구현계획_20260912b.md`). 부수적으로 b→c 개정 때부터 남아있던 절 번호 참조 오류(갱신 규칙 표와 6절 "참고"의 "6절" → "7절")를 바로잡았다. 그 외 새로운 결정은 텔레그램 호스트 관련 항목뿐이다.
...
핵심 규칙: **MCP와 CLI는 반드시 Core를 거친다.** (d판 추가) **텔레그램 호스트도 다른 호스트와 동일하게 MCP를 거쳐 Core에 접근한다 — Core를 직접 열지 않는다.**
...
- [ ] 14. (d판 신설) 텔레그램 호스트가 화이트리스트 없는 chat에 응답하거나, 읽기 이외의 툴(쓰기/삭제)을 노출하고 있지는 않은가
...
### Phase 11 — Telegram 원격 호스트 (d판 신설, v1: 읽기 전용)

- [ ] `apps/telegram-bot`가 `TELEGRAM_BOT_TOKEN`/`TELEGRAM_OWNER_CHAT_ID` 환경변수 없이는 기동하지 않음
- [ ] 화이트리스트 chat id가 아닌 메시지는 응답 없이 무시 + 로그만 남김
- [ ] 봇 프로세스가 노출하는 MCP 툴은 Phase 6/7의 5종(`list_projects`/`get_atlas`/`get_graph`/`get_project_bundle`/`get_progress`)뿐 — 쓰기 툴은 애초에 MCP 서버에 없으므로 별도 필터링 로직 불필요
- [ ] long-polling 방식, 웹훅 없음. 자동 기동(부팅 시 시작 등) 없음 — 사용자가 수동으로 기동
- [ ] 대화 히스토리는 프로세스 메모리에만 유지, 재시작 시 소실. 파일/DB에 영속화하지 않음
...
| Q15 | PortId slug/fileRef 모호성 | `skill:` fileRef는 반드시 `SKILL.md`로 끝나야 함(6.1절) — 확장자 없는 이름은 항상 Bay slug |
| Q16 (d판 신설) | 텔레그램 호스트 배포 방식 | v1은 long-polling, 사용자가 개인 머신에서 수동 기동. 웹훅·상시 클라우드 배포·다중 사용자/그룹챗은 보류 |
...
- 어떤 순서로 만들지 → `Patchbay_구현계획_20260912b.md` (7절과 충돌하면 이 문서 7절)
- 지금 맞게 가고 있는지 → 이 문서
...
- **Phase 7**: `get_progress` — progress write CLI와 같은 시점에 추가한다. b판에서 이 툴이 통째로 빠져 있었다(8절 참고).
...
- 권한: 읽기 전용. Phase 6/7에서 이미 정의된 MCP 5종만 노출 — `patch_skill`/`record_decision`처럼 아직 없는 쓰기 툴은 텔레그램에도 없음(7.12절과 동일 제약을 상속받음)
- 상태: 대화 히스토리는 프로세스 메모리에만 존재, 재시작 시 소실. `progress.yaml`이나 다른 studio 파일에 쓰지 않음
- 실패 처리: 토큰 만료/네트워크 오류는 텔레그램 답장으로 노출, 프로세스는 유지
- 원칙 10과의 관계: 이 프로세스는 "상시 켜져 있어야" 동작하는 것이 기능의 본질(텔레그램 수신 대기)이므로 **원칙 10(상시 워치/서버 금지)의 명시적 예외**로 둔다. 예외 조건: (a) 수동 기동만 허용, (b) 읽기 전용, (c) studio 파일을 쓰지 않음 — 이 세 조건을 벗어나면 원칙 10 위반으로 간주한다.
...
4. **기획서에 텔레그램 호스트 미반영 (d판 신설)** — 이 문서 1.1절/7.16절에서 텔레그램 봇을 새 상시 호스트로 결정했지만, `Patchbay_기획_20260912c.md`의 호스트 목록·아키텍처 서술에는 아직 없다. 기획서 다음 개정(d판)에서 반영 필요.
...
| 20260912d | 2026-09-13 | 텔레그램 원격 호스트(`apps/telegram-bot`, v1 읽기 전용) 신설: 아키텍처(1.1)·원칙 자가점검(14번)·Phase 11 체크리스트/진행흐름·Q16·7.16 채택값·원칙 10 예외 조건 추가. 절 번호 참조 오류(6→7) 수정 |
