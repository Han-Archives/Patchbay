...
| 문서명 | Patchbay_명세 |
| 버전 | 20260913b |
| 이전 버전 | `Patchbay_명세_20260913a.md` |
...
20260913b판 변경 요지: **Phase 0~3을 실제로 구현·검토·머지했다**(커밋 `bca1f1b`..`cb60795`, 요청서/검토 구조로 진행). 그 과정에서 나온 두 가지 실행 결정을 반영 — (1) studio 홈 초기화를 Phase 0→Phase 3으로 이동(구현계획 20260913b판과 동기화), (2) `ATLAS.md`의 `## Skills`/`## Rules`/`## Unlabeled ports` 콘텐츠 매핑을 7.18절에 신규 확정. Phase 0~3 체크리스트에 완료 표시를 남겼다.
...
- (`packages/core` 137 tests + `packages/cli` 39 tests, 전체 빌드 클린. 모든 테스트가 `PATCHBAY_HOME`을 임시 디렉터리로 강제 — 실제 `~/.patchbay`는 건드리지 않음을 검토자가 직접 확인)
