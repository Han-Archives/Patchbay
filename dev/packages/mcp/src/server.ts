...
/**
 * Patchbay's MCP wrapper (spec Phase 6): exactly four read-only tools --
 * `list_projects`, `get_atlas`, `get_graph`, `get_project_bundle` -- all
 * built on `@patchbay/core`. No `patch_skill`/`record_decision`/
 * `get_progress` here (later phases); no LLM calls.
 *
